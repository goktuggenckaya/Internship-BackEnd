...
rfts.sira as 'order',
rfts.soru as 'content(fulltext)'
from rfen_test_sorular rfts

select soru as 'content', COUNT(soru)
from rfen_test_sorular
group by soru 



select md5(nt.content) as 'hash', COUNT(md5(nt.content)) as 'num of content'
from (select ats.id as Table_id, 'Akilli Test Sorulari' as Table_name, ats.uniteid as unit_id,
ats.testid as test_id, ats.video as solution_video_id, ats.sira as 'Order', ats.soru as 'Content' from akilli_test_sorular ats
union all
select tats.id as Table_id, 'Tarih Akilli Test Sorulari' as Table_name, tats.uniteid as unit_id,
tats.testid as test_id, tats.video as solution_video_id, tats.sira as 'Order', tats.soru as 'Content' from tarih_akilli_test_sorular tats
union all
select tts.id as Table_id, 'Tarih Testi Soruları' as Table_name, '-' as unit_id,'-' as test_id,
tts.video as solution_video_id,'-' as 'Order', tts.soru as 'Content' from tarih_test_soru tts
union all
select
ts.id as Table_id,
...
from rfen_test_sorular rfts) as nt
group by md5(nt.content)
...
rfts.soru as 'Content'
from rfen_test_sorular rfts) as nt
where solution_video_id is null 
